                ...
                async {
                    ...
                    try {
                        ...
                        if (targetUrl.contains("streamc.xyz") || targetUrl.contains("phimmoi.net")) {
                            ...
                            // ── BƯỚC 1: Fetch embed HTML → extract token ──
                            val html = fetchEmbedHTML(targetUrl, embedDomain)
                            if (html != null) {
                                val obfData = extractObfData(html)
                                if (obfData != null) {
                                    token = obfData.first   // sUb
                                    kX    = obfData.third   // kX (thường null — tính dynamic trong player.js)
                                    val decoded = token?.let { decodeStreamcToken(it) }
                                    println("[NguonC] token: h=${decoded?.second?.take(16)}... t=${decoded?.third?.take(16)}... kX=$kX")
                                }
                                if (token == null) token = findTokenInHTML(html)
                                // KHÔNG gọi findEncryptionKey: player.js là 156KB minified bundle,
                                // kX tính dynamically — static scan luôn fail và waste thời gian
                            }
                            ...
                            if (!linkFound) {
                                ...
                                try {
                                    ...
                                    if (content.contains(\"#ENC-AESGCM\")) {
                                        ...
                                        // Log full content để xác định format thực tế
                                        println("[NguonC] m3u8 content: ${content.take(800)}")
                                        println("[NguonC] capToken=${capToken?.take(40)}")

                                        // Thử A: nếu m3u8 có #EXTM3U → có thể là HLS chuẩn (#EXT-X-KEY)
                                        // ExoPlayer tự xử lý AES-128 segment decryption
                                        if (!linkFound && content.contains("#EXTM3U")) {
                                            println("[NguonC] Has #EXTM3U → try as standard HLS")
                                            val base = if (capturedUrl.isNotEmpty()) capturedUrl.substringBeforeLast("/") + "/" else "$embedDomain/"
                                            if (registerM3U8Link(content, targetUrl, base, serverName, callback)) {
                                                println("[NguonC] Passed as standard HLS")
                                                linkFound = true; return@async
                                            }
                                        }

                                        // Thử B: AES-GCM decrypt với tất cả key candidates từ token
                                        // kX null → decryptStreamcM3u8 vẫn thử t/h derived keys
                                        if (!linkFound) {
                                            val decrypted = decryptStreamcM3u8(content, kX ?: "", capToken ?: token)
                                            if (!decrypted.isNullOrEmpty() && decrypted.contains("#EXTM3U")) {
                                                println("[NguonC] Decrypt OK!")
                                        ...
                                        if (!linkFound) println("[NguonC] DECRYPT FAILED kX=$kX")
                                    }
                                } catch (e: Exception) {
