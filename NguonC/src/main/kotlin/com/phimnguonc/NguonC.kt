...
class PhimNguonCProvider : MainAPI() {
    ...
    private fun decryptStreamcM3u8(content: String, kX: String, token: String? = null): String? {
        try {
            val ivMatch = Regex("""#ENC-AESGCM;iv=([a-f0-9A-F]+)""").find(content)
            val iv: ByteArray
            if (ivMatch != null) {
                val ivHex = ivMatch.groupValues[1]
                iv = hexToBytes(ivHex)
                println("[NguonC] Decrypt: iv=${ivHex.take(24)} (${iv.size}B) kX=${kX.take(16)}")
            } else if (token != null) {
                // Không có iv= trong content → thử dùng t cuối 12/16 bytes làm IV
                val decoded = decodeStreamcToken(token)
                val t = decoded?.third ?: ""
                iv = if (t.length >= 24) hexToBytes(t.takeLast(24)) // last 12 bytes
                     else ByteArray(12)   // zero IV
                println("[NguonC] Decrypt: no iv= in m3u8, using t-tail iv (${iv.size}B)")
            } else {
                println("[NguonC] Decrypt: no iv= and no token → skip")
                return null
            }
            ...
            if (token != null) {
                ...
                if (decoded != null) {
                    ...
                    // ── Pattern phổ biến nhất site Việt Nam: t/h dưới dạng ASCII string bytes ──
                    // kX = t.substring(0, 32) as UTF-8 bytes (AES-256, 32 ASCII chars = 32 bytes)
                    if (t.length >= 32) keyCandidates.add(Pair("t-str-utf8-32", t.take(32).toByteArray(Charsets.UTF_8)))
                    // kX = t.substring(0, 16) as UTF-8 bytes (AES-128)
                    if (t.length >= 16) keyCandidates.add(Pair("t-str-utf8-16", t.take(16).toByteArray(Charsets.UTF_8)))
                    // kX = h as UTF-8 bytes (h = 32 hex chars → 32 bytes = AES-256 key)
                    keyCandidates.add(Pair("h-str-utf8-32", h.toByteArray(Charsets.UTF_8)))
                    // kX = h first 16 chars as UTF-8 (AES-128)
                    if (h.length >= 16) keyCandidates.add(Pair("h-str-utf8-16", h.take(16).toByteArray(Charsets.UTF_8)))
                    // kX = full t string as UTF-8 truncated to 64 bytes - một số site dùng t trực tiếp
                    val tUtf8 = t.toByteArray(Charsets.UTF_8)
                    if (tUtf8.size >= 32) keyCandidates.add(Pair("t-str-utf8-full32", tUtf8.copyOf(32)))
                    if (tUtf8.size >= 16) keyCandidates.add(Pair("t-str-utf8-full16", tUtf8.copyOf(16)))
    ...
    private suspend fun tryFetchWithToken(
        ...
        // Decode token để lấy t → dùng làm ?xat= session proof
        val decoded = decodeStreamcToken(token)
        val xat     = decoded?.third  // t field = session token

        // URL-safe base64 của token (standard cho streamc.xyz)
        val urlSafeToken = toUrlSafeBase64(token)

        for (ext in listOf(".m3u9", ".m3u8")) {
            // ?xat={t} là session proof — SERVER YÊU CẦU — không có → Unauthorized
            val m3u8Url = if (xat != null) "$embedDomain/$urlSafeToken$ext?xat=$xat"
                          else             "$embedDomain/$urlSafeToken$ext"
            println("[NguonC] Fetching: ${m3u8Url.take(100)}")

            val m3u8Content = fetchDirect(m3u8Url, referer, embedDomain) ?: continue

            // Plain m3u8 (expected - streamc dùng plain HLS, segments có .png extension)
            if (m3u8Content.contains("#EXTM3U") && !m3u8Content.contains("#ENC-AESGCM")) {
                println("[NguonC] Plain m3u8 OK ($ext)!")
                val base = "$embedDomain/"
                if (registerM3U8Link(m3u8Content, referer, base, serverName, callback)) {
                    return true
                }
            }
            // Fallback: encrypted
            if (m3u8Content.contains("#ENC-AESGCM")) {
                ...
                if (!decrypted.isNullOrEmpty() && decrypted.contains("#EXTM3U")) {
                    if (registerM3U8Link(decrypted, referer, "", serverName, callback)) return true
                }
            }
        }
        return false
            ...
            embedEntries.map { (serverName, url) ->
                async {
                    ...
                    try {
                        ...
                        // ══════════════════════════════════════════════════════════════
                        // STREAMC.XYZ — SIMPLIFIED FLOW (từ phân tích dữ liệu thực):
                        // 1. Fetch embed HTML → extract sUb token
                        // 2. Decode sUb → lấy t (session proof)
                        // 3. Direct fetch: {embedDomain}/{sUb_urlsafe}.m3u9?xat={t}
                        //    ?xat={t} là session proof — SERVER YÊU CẦU THAM SỐ NÀY
                        // 4. m3u8 là PLAIN HLS (không mã hóa), segments dùng .png extension
                        // NOTE: Không cần WebView, không cần decrypt!
                        // ══════════════════════════════════════════════════════════════
                        if (targetUrl.contains("streamc.xyz") || targetUrl.contains("phimmoi.net")) {
                            println("[NguonC] === Processing: ${targetUrl.take(80)} ===")

                            // BƯỚC 1: Fetch embed HTML → extract token (sUb)
                            val html = fetchEmbedHTML(targetUrl, embedDomain)
                            val token = html?.let { h ->
                                val obf = extractObfData(h)
                                val t   = obf?.first
                                if (t != null) {
                                    val dec = decodeStreamcToken(t)
                                    println("[NguonC] sUb: h=${dec?.second?.take(16)}... t=${dec?.third?.take(16)}...")
                                }
                                t ?: findTokenInHTML(h)
                            }

                            if (token == null) {
                                println("[NguonC] FAILED: no token found")
                                return@async
                            }

                            // BƯỚC 2: Direct fetch với ?xat={t}
                            // tryFetchWithToken đã được update để thêm ?xat={t}
                            if (tryFetchWithToken(token, embedDomain, targetUrl, null, serverName, callback)) {
                                linkFound = true; return@async
                            }

                            // BƯỚC 3: WebView fallback (chỉ khi direct fetch fail)
                            if (!linkFound) {
                                println("[NguonC] Direct fetch failed, trying WebView fallback...")
                                try {
                                    val resp = app.get(targetUrl, interceptor = m3u8Interceptor, headers = mapOf(
                                        "Referer" to "$mainUrl/", "User-Agent" to USER_AGENT
                                    ))
                                    val wContent = resp.text
                                    val wUrl     = resp.url ?: ""
                                    println("[NguonC] WebView m3u8 (${wContent.length}B): ${wContent.take(200)}")
                                    if (wContent.contains("#EXTM3U")) {
                                        val base = if (wUrl.isNotEmpty()) wUrl.substringBeforeLast("/") + "/" else "$embedDomain/"
                                        if (registerM3U8Link(wContent, targetUrl, base, serverName, callback)) {
                                            linkFound = true; return@async
                                        }
                                    }
                                } catch (e: Exception) {
                                    println("[NguonC] WebView fallback failed: ${e.message}")
                                }
                            }

                            if (!linkFound) println("[NguonC] FAILED all approaches: $targetUrl")
                            return@async
                        }
                            ...
                            for (m3u8Url in urlsToTry) {
                                ...
                                if (content != null) {
                                    ...
                                    if (content.contains("#ENC-AESGCM")) {
                                        ...
                                        val decrypted = decryptStreamcM3u8(content, "", urlToken)
                                        if (!decrypted.isNullOrEmpty() && decrypted.contains("#EXTM3U")) {
                        ...
                        if (m3u8Url == null && (otherHtml.contains("streamc") || otherHtml.contains("jwplayer") || otherHtml.contains("data-obf"))) {
                            ...
                            if (extractedToken != null) {
                                val dec = decodeStreamcToken(extractedToken)
                                val xat = dec?.third
                                m3u8Url = if (xat != null)
                                    "$embedDomain/${toUrlSafeBase64(extractedToken)}.m3u9?xat=$xat"
                                else
                                    "$embedDomain/${toUrlSafeBase64(extractedToken)}.m3u9"
                            }
                        }
                        ...
                        if (!m3u8Url.isNullOrEmpty() && !m3u8Url.startsWith("blob:")) {
                            ...
                            if (m3u8Content != null) {
                                if (m3u8Content.contains("#EXTM3U") && !m3u8Content.contains("#ENC-AESGCM")) {
                                    ...
                                    val decrypted = decryptStreamcM3u8(m3u8Content, extractedKX ?: "", extractedToken)
                                    if (!decrypted.isNullOrEmpty() && decrypted.contains("#EXTM3U")) {
                                        registerM3U8Link(decrypted, targetUrl, "", serverName, callback)
                                        linkFound = true
                                    }
